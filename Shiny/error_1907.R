  ...
  tabsetPanel(               
    ...
    tabPanel("Tomato", h1("Tomatoproduktion"),
             ...
             fluidRow(
               ...
               sidebarPanel(width = 6,
                            h2(strong('Anwendung')),
                            h5 ("1. Bitte geben Sie Ihre Kgeschäzte Kennzahlen ein"),
                            h5 ("2. Klicken Sie 'Speichern'"),
                            h5 ("3. Klicken Sie 'Download', sodass ein CSV file in Ihrem Computer/ Laptop herungergeladen wird"),
                            h5 ("4. Senden Sie bitte an 's62smoon@uni-bonn.de'")
                            
               )
             ),
             fluidRow(h1(strong('Kosten')),
                      sidebarPanel(width = 6,
                             ...
                             fluidRow(
                               sidebarPanel(width = 12,  
                                            ...
                                            sliderInput("Dienstleistungen",
                                                        ...
                                                        max = 10000,
                                                        value =  c(6500,6800),
                                                        step=100))),# closed fluidRow
                             sidebarPanel(width = 12,
                                          fluidRow( h3( strong('Ergebnisse')),
                                                    plotOutput("distPlot1",height = "1000px",
                                                               width = "95%")))
                      )
             ),
             fluidRow(
...
server <- function(input, output) {
  ...
  output$distPlot1 <- renderPlot({
    ...
    input_estimates <- data.frame(variable = c("Ertrag","Tomaten_Preis","Priming_Effect",
                                               "Jungpflanzen_Menge","Jungpflanzen_Preis","Priming_Jungpflanzen",
                                               "Du_Bewaesserung", "Heizmaterial", "Strom",
                                               "Matten_Folien","CO2","Pflanzenschutz",
                                               "Verpackung", "Transport","Entsorgung_Menge", 
                                               "Entsorgung_Preis", "Re_Biowaste","Vermarktungsgebuehr", 
                                               "sonstige_Betriebsmittel", "fix_Lohnkosten","variable_Kosten",
                                               "Dienstleistungen", "discount_rate","n_years", 
                                               "var_CV","num_simulations"),
                                  lower = c(min(input$Ertrag), min(input$Tomaten_Preis),min(input$Priming_Effect),
                                            ...
                                            min(input$variable_Kosten), min(input$Dienstleistungen),3, 10,20,10),
                                  median = NA,
                                  upper = c(max(input$Ertrag), max(input$Tomaten_Preis),max(input$Priming_Effect),
                                            ...
                                            max(input$Verpackung), max(input$Transport),max(input$Entsorgung_Menge),
                                            max(input$Entsorgung_Preis),max(input$Re_Biowaste),max(input$Vermarktungsgebuehr), 
                                            max(input$sonstige_Betriebsmittel),max(input$fix_Lohnkosten), max(input$variable_Kosten), 
                                            max(input$Dienstleistungen),3,10,
                                            20,10),
                                  distribution = c("posnorm", "posnorm", "posnorm", "posnorm",
                                                   ...
                                                   "posnorm", "posnorm", "posnorm", "posnorm",
                                                   "posnorm",  "const","const","const","const"),
                                  ...
                                  label = c("Ertrag (kg/m2)","Tomaten Preis (€/kg)","Effekte von Priming auf Ertrag(%)",
                                            ...
                                            "coeff. Variation","Simulationsnummer"))
  ...
  Data <- data.frame()
  Results <- reactive(data.frame(input$Datum,input$Betrieb, input$name,
                                 input$Ertrag, input$Tomaten_Preis,input$Priming_Effect,
                                 input$Jungpflanzen_Menge, input$Jungpflanzen_Preis,input$Priming_Jungpflanzen,
                                 input$Du_Bewaesserung, input$Heizmaterial,input$Strom,
                                 input$Matten_Folien, input$CO2,input$Pflanzenschutz,
                                 input$Verpackung, input$Transport,input$Entsorgung_Menge,input$Entsorgung_Preis,input$Re_Biowaste,
                                 input$Vermarktungsgebuehr, input$sonstige_Betriebsmittel,input$fix_Lohnkosten,
                                 input$variable_Kosten, input$Dienstleistungen,input$discount_rate,
                                 input$n_years,input$var_CV ,input$Verbesserung, Sys.Date()))  
